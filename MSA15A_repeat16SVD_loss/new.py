...
tf_keep = tf.placeholder(tf.int32,[None],name='keep_svd')
window = 5
num1 = 64/4
num2 = 96/4
num3 = 192/4
...
for epoch in range(next_epoch,training_epochs):
    ...
    if True:
        for batch in range(0,len(shuffle),num):
            ...
            keep_row_col = [xx for xx in range(batch_y_nan.shape[1]) if np.mean(batch_y_nan[0,xx,:,0])!=0]
    ...
    if True:
        ...
        for i in range(0,len(data2_x)):
            ...
            batch_x = np.swapaxes(np.swapaxes(batch_x,1,3),1,2)
            keep_row_col = [x for x in range(batch_y_nan.shape[1]) if np.mean(batch_y_nan[0,x,:,0])!=0]
