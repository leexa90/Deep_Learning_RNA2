...
# identity matrix for orthorgonal optimization
Identity_matrix = tf.placeholder(tf.float32, [1,None,None],name='I_matrix')
window = 5
num1 = 128/4
num2 = 96/4
num3 = 192/4
...
weights = {
    ...
    '2_svd': tf.Variable(tf.random_normal([3, 3, num2*4, num2*4])),
    '2_diag': tf.Variable(tf.random_normal([3, 3, num2*4, num2*4])),
...
biases = {
    ...
    '2_svd' : tf.Variable(tf.random_normal([num2*4])),
    '2_diag' : tf.Variable(tf.random_normal([num2*4])),
...
conv2_diag = conv2d(conv2,weights['2_diag'],biases['2_diag'],relu=False)
conv2_svd = conv2d(conv2,weights['2_svd'],biases['2_svd'],relu=False) #get V.T/U matrix
conv2_diag = conv2d(conv2,weights['2_diag'],biases['2_diag'],relu=False)

#orthorgonal optimzation on conv2
W = conv2_svd[:,0,:,:]
WW_t= tf.matmul(W, tf.transpose(W,(0,2,1)))
orthorgonal_loss = tf.reduce_mean(tf.squared_difference(WW_t, Identity_matrix),name='orthorgonal_loss')
...
for i in range(0,num2*4): #reconstruction of image from SVD
    diag  = tf.diag(conv2_diag[:,0,:,i])[:,:,0,:] #batch size must be 1 because of here
    ud = tf.matmul(diag[:,:,:],W)
    svd = tf.matmul(ud, tf.transpose(W,(0,2,1))) 
    final += [svd,]
...
with tf.control_dependencies(update_ops):
    ...
    orthorgonal_optimzer = tf.train.MomentumOptimizer(learning_rate=0.001,momentum=0.5).minimize(orthorgonal_loss)
...
for epoch in range(next_epoch,training_epochs):
    ...
    if True:
        for batch in range(0,len(shuffle),num):
            ...
            _, c = sess.run([extra_optimizer, orthorgonal_optimzer], feed_dict={x: batch_x,
                                                          resi_map0: batch_y,
                                                          above_zero : batch_y_nan,
                                                          ss_2d : batch_y_ss,
                                                          phase : True, learning_rate : lr,
                                                          dropout : 0.0,Identity_matrix : np.array([np.eye(batch_y_nan.shape[1])])})
