...
resi_map0 = tf.placeholder(tf.float32,[None,None,None,1],name='distance_mat')
# some values are na, these will be excluded from loss
above_zero = tf.placeholder(tf.float32,[None,None,None,1],name='above_zero')
...
##conv1p2 = tf.concat([conv1ac[:,:,:,1::2],conv1bb[:,:,:,1::2],conv1cb[:,:,:,1::2],conv1da[:,:,:,1::2]],3)
##conv1 =  tf.concat([average_pooling2d(conv1p1 ,  (1,2),1,padding='same'),
##                    max_pooling2d(conv1p2 ,  (1,2),1,padding='same')],3)
conv1 = tf.layers.dropout(conv1,rate=dropout,name='drop1')
...
##conv2p2 = tf.concat([conv2ac[:,:,:,1::2],conv2bb[:,:,:,1::2],conv2cb[:,:,:,1::2],conv2da[:,:,:,1::2]],3)
##conv2 =  tf.concat([average_pooling2d(conv2p1 ,  (1,2),1,padding='same'),
##                    max_pooling2d(conv2p2 ,  (1,2),1,padding='same')],3)
...
##conv3p1 = tf.concat([conv3ac[:,:,:,0::2],conv3bb[:,:,:,0::2],conv3cb[:,:,:,0::2],conv3d[:,:,:,0::2]],3)
##conv3p2 = tf.concat([conv3ac[:,:,:,1::2],conv3bb[:,:,:,1::2],conv3cb[:,:,:,1::2],conv3d[:,:,:,1::2]],3)
##conv3 =  tf.concat([average_pooling2d(conv3p1 ,  (2,2),1,padding='same'),
##                    max_pooling2d(conv3p2 ,  (2,2),1,padding='same')],3)
...
##conv4p1 = tf.concat([conv4ac[:,:,:,0::2],conv4bb[:,:,:,0::2],conv4cb[:,:,:,0::2],conv4d[:,:,:,0::2]],3)
##conv4p2 = tf.concat([conv4ac[:,:,:,1::2],conv4bb[:,:,:,1::2],conv4cb[:,:,:,1::2],conv4d[:,:,:,1::2]],3)
##conv4 =  tf.concat([average_pooling2d(conv4p1 ,  (2,2),1,padding='same'),
##                    max_pooling2d(conv4p2 ,  (2,2),1,padding='same')],3)
...
##conv5p1 = tf.concat([conv5ac[:,:,:,0::2],conv5bb[:,:,:,0::2],conv5cb[:,:,:,0::2],conv5d[:,:,:,0::2]],3)
##conv5p2 = tf.concat([conv5ac[:,:,:,1::2],conv5bb[:,:,:,1::2],conv5cb[:,:,:,1::2],conv5d[:,:,:,1::2]],3)
##conv5 =  tf.concat([average_pooling2d(conv5p1 ,  (2,2),1,padding='same'),
##                    max_pooling2d(conv5p2 ,  (2,2),1,padding='same')],3)
conv5 = tf.layers.dropout(conv5,rate=dropout)
### 2d third inception layer
...
##conv6p1 = tf.concat([conv6ac[:,:,:,0::2],conv6bb[:,:,:,0::2],conv6cb[:,:,:,0::2],conv6d[:,:,:,0::2]],3)
##conv6p2 = tf.concat([conv6ac[:,:,:,1::2],conv6bb[:,:,:,1::2],conv6cb[:,:,:,1::2],conv6d[:,:,:,1::2]],3)
##conv6 =  tf.concat([average_pooling2d(conv6p1 ,  (2,2),1,padding='same'),
##                    max_pooling2d(conv6p2 ,  (2,2),1,padding='same')],3)
##conv6 = tf.layers.dropout(conv6,rate=dropout)
### 2d third inception layer
...
##conv7p1 = tf.concat([conv7ac[:,:,:,0::2],conv7bb[:,:,:,0::2],conv7cb[:,:,:,0::2],conv7d[:,:,:,0::2]],3)
##conv7p2 = tf.concat([conv7ac[:,:,:,1::2],conv7bb[:,:,:,1::2],conv7cb[:,:,:,1::2],conv7d[:,:,:,1::2]],3)
##conv7 =  tf.concat([average_pooling2d(conv7p1 ,  (2,2),1,padding='same'),
##                    max_pooling2d(conv7p2 ,  (2,2),1,padding='same')],3)
##conv7 = tf.layers.dropout(conv7,rate=dropout)
...
for chkpt in [ 'model300_reweigh_loss_148_79.ckpt',
              'model300_reweigh_loss_262_79.ckpt','model300_reweigh_loss_262_79.ckpt']:
    saver.restore(sess,chkpt)
    for i in range(len(data2_x_test)):
            if data2_name_test[i] not in test_acc:
                test_acc[data2_name_test[i]]= [np.array([[data2_x_test[i],],]),np.array([data2_y_test[i]]),]
            ...
            pred =sess.run( out_softmax, feed_dict={x: batch_x,resi_map0: batch_y,
                                                 above_zero : batch_y_nan, ss_2d : batch_y_ss,
                                                    phase : False, learning_rate : lr})
            #test_acc[data2_name_test[i]] += [accuracy(np.argmax(pred+np.transpose(pred,(0,2,1,3)),3)[0],np.argmax(batch_y,3)[0]),]
            test_acc[data2_name_test[i]] += [(pred+np.transpose(pred,(0,2,1,3)))[0,:,:,0]//1,]

            

dictt_AUGC={(0, 0, 0, 1): 'C', (1, 0, 0, 0): 'A', (0, 0, 1, 0): 'G', (0, 1, 0, 0): 'U'}
if False: #makes the weights
    for key in  test_acc:
        f1 = open('weights_%s'%key,'w')
        f1.write('[ atompairs ]\n')
        pred = np.transpose(np.array(test_acc[key][2:5]), (1,2,0))
        seq = test_acc[key][0][0,0,:4,:]
        seq_string = ''
        for i in range(len(pred)):
            seq_string += dictt_AUGC[tuple(seq[:,i])]
        for i in range(len(pred)):
            for j in range(i+1,len(pred)):
                if np.mean(pred[i,j]) <= 0.5:
                    dist = '%s %s %s %s FLAT_HARMONIC %s 1 8\n' %('P',i+1,'P',j+1,8)
                    f1.write(dist)
        f1.close()
die
...
def accuracy2(mat_model,answer):
    score = [[0],[0],[0]]
    for i in range(0,answer.shape[1]):
        if np.sum(answer[i,:]) != 0:
            for j in range(i+1,answer.shape[1]):
                if np.sum(answer[j,:]) != 0:
                    if answer[i,j] == mat_model[i,j]:
                        ...
                        score[answer[i,j]] += [0,]
    print [np.mean(score[0]),np.mean(score[1]),np.mean(score[2])]
    return np.mean([np.mean(score[0]),np.mean(score[1]),np.mean(score[2])])
saver.restore(sess,'model300_reweigh_loss_148_79.ckpt')
...
for i in range(len(data2_x)):
    ...
    pred =sess.run( out_softmax, feed_dict={x: batch_x,resi_map0: batch_y,
                                         above_zero : batch_y_nan, ss_2d : batch_y_ss,
                                            phase : False, learning_rate : lr, dropout : 1})
    pred = (pred+np.transpose(pred,(0,2,1,3)))[0,:,:,0]//1
    actual = batch_y[0,:,:,0]//1
    val_acc += [accuracy(pred,actual),]
    f, ax = plt.subplots(2,1,figsize=(7,10))
    ax[0].imshow(pred)
    ax[0].set_xlabel(data2_name[i])
    ax[1].set_xlabel(val_acc[-1])
    ax[1].imshow(batch_y[0,:,:,0])
    plt.savefig('TRAIN/Train_'+data2_name[i]+'.png')
    plt.clf()
    plt.close()
...
for i in range(len(data2_x_val)):
    ...
    ax[1].set_xlabel(val_acc[-1])
    ax[1].imshow(batch_y[0,:,:,0])
    plt.savefig('VAL/Val_'+data2_name_val[i]+'.png')
    plt.clf();plt.close()
test_acc2 = {}
data2_t = np.load('../data_lim5000_ss.npy.zip')['data_lim5000_ss'].item()
for i in test_acc :
    ...
    for ii in ss:
        if ii == -1:
            ...
            ss_str += '.'
    test_acc2[i] = np.median(np.stack(test_acc[i][1:]),0)
    ensemble = test_acc2[i]
    answer = test_acc[i][0][0,:,:,0]
    print (i,accuracy(ensemble,answer))
    f, ax = plt.subplots(3,2,figsize=(10,10))
    ax[0,0].imshow(test_acc[i][1])
    ax[0,1].imshow(test_acc[i][2])
    ax[1,0].imshow(test_acc[i][3])
    ax[1,1].imshow(ensemble)
    ax[2,0].imshow(answer)
    ax[2,1].set_xlabel(i)
    ax[2,0].set_xlabel(ss_str)
    ax[0,0].set_xlabel(accuracy2(ensemble,answer))
    plt.savefig(i+'.png')
    plt.clf();plt.close()
