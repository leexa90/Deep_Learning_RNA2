...
data2_t = np.load('../data_lim5000_ss2.npy.zip')['data_lim5000_ss2'].item()
data3_t = np.load('../data_lim5000_extra.npy').item()
data4_t = np.load('../data_lim5000_MSA.npy').item()

data1 = np.load('../data_lim5000_nan_composer.npy.zip')['data_lim5000_nan_composer.npy'].item()
data2 = np.load('../data_lim5000_ss_composer2.npy.zip')['data_lim5000_ss_composer2'].item()
data3 = np.load('../data_lim5000_extra_composer.npy').item()
data4 = np.load('../data_lim5000_MSA_composer.npy').item()
...
data1_keys_val = [x.lower() for x in ['4PR6_B', '1DK1_B', '1EXD_B', '1FFY_T', '1I6U_C',
...
print ('training samples %s from %s PDBid, val samples %s from %s PDBid, test samples %s from %s PDBid,' %(train_n,len(data1_keys_train),val_n,len(data1_keys_val),test_n,len(data1_keys_test)))            
...
def dice_coe(output, target, loss_type='jaccard', axis=[1,2,3], smooth=1e-5,above_zero=above_zero):
    ...
    above_zero : same size as output
        this is to mask the loss for missing pixels
    Examples
    ---------
    ...
    """
    inse = tf.reduce_sum(output * target * above_zero, axis=axis) # TP
    if loss_type == 'jaccard':
        l = tf.reduce_sum(output * output * above_zero, axis=axis) # TP + FP
        r = tf.reduce_sum(target * target * above_zero, axis=axis) # TP + FN
    elif loss_type == 'sorensen':
        l = tf.reduce_sum(output * above_zero, axis=axis)
        r = tf.reduce_sum(target * above_zero, axis=axis)
    else:
    ...
    ## new haodong
    dice = (2. * inse + smooth) / (l + r + smooth) # 2TP / (2TP + FP + FN)
    ##
    dice = tf.reduce_mean(dice) #edited by XA
    return dice
...
cost = dice_coe(out_softmax,resi_map)
