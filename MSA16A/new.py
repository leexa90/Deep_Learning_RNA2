...
weight_coeff1 = 0.5*weight1 
weight_coeff2 = 0.5*weight2
...
logit_weight = tf.constant([weight_coeff1,weight_coeff2],tf.float32)
...
for epoch in range(next_epoch,training_epochs):
    ...
    for batch in range(0,len(shuffle[::10]),num):
        ...
        counter += 1
        lr = 0.005*np.cos(0.5*batch*3.142/len(shuffle[::10]))    
        ...
        _, c = sess.run([extra_optimizer, cost], feed_dict={x: batch_x,
                                                      ...
                                                      ss_2d : batch_y_ss,
                                                      phase : True, learning_rate : lr, dropout : 0.})
