...
weights = {
    ...
    '3_wc1aa': tf.Variable(tf.random_normal([1, 1, num2*8+64, num3])),
    '3_wc1ab': tf.Variable(tf.random_normal([window, 1, num3, num3])),
    '3_wc1ac': tf.Variable(tf.random_normal([1, window, num3, num3])),
    '3_wc1ba': tf.Variable(tf.random_normal([1, 1, num2*8+64, num3])),
    '3_wc1bb': tf.Variable(tf.random_normal([3, 3, num3, num3])),
    '3_wc1c': tf.Variable(tf.random_normal([1, 1, num2*8+64, num3])),
    '3_wc1d': tf.Variable(tf.random_normal([1, 1, num2*8+64, num3])),
...
for i in range(num2*4):
    diag = tf.diag(conv2_diag[:,0,:,i])[:,:,0,:]
    ud = tf.matmul(diag[:,:,:],conv2_svd[:,0,:,:])
    svd = tf.matmul(ud, tf.transpose(conv2_svd[:,0,:,:],(0,2,1)))
    final += [svd,]
