...
weight1 = 1.0#(classweight1 / (classweight1+classweight2+classweight3))**-1
weight2 = 1.0#(classweight2 / (classweight1+classweight2+classweight3))**-1
weight3 = 1.0#(classweight3 / (classweight1+classweight2+classweight3))**-1
...
resi_map0 = tf.placeholder(tf.float32,[None,None,None,3],name='distance_mat')
# some values are na, these will be excluded from loss
above_zero = tf.placeholder(tf.float32,[None,None,None,3],name='above_zero')
...
conv1p1 = tf.concat([conv1ac[:,:,:,0::2],conv1bb[:,:,:,0::2],conv1cb[:,:,:,0::2],conv1da[:,:,:,0::2]],3)
...
conv2p1 = tf.concat([conv2ac[:,:,:,0::2],conv2bb[:,:,:,0::2],conv2cb[:,:,:,0::2],conv2da[:,:,:,0::2]],3)
...
for chkpt in [xx[:-5] for xx in os.listdir('.') if xx.endswith('ckpt.meta')]:
    saver.restore(sess,chkpt)
    for i in range(len(data2_x_test)):
            if data2_name_test[i] not in test_acc:
                test_acc[data2_name_test[i]]= [np.array([data2_y_test[i]]),]
            ...
            test_acc[data2_name_test[i]] += [pred/2+np.transpose(pred,(0,2,1,3))/2,]
...
if Test:
    test_acc2 = {}
    for i in test_acc :
        test_acc2[i] = np.median(np.concatenate(test_acc[i][1:]),0)
        ensemble = np.argmax(test_acc2[i],2)
        answer = np.argmax(test_acc[i][0][0],2)
        print (i,accuracy(ensemble,answer))
        f, ax = plt.subplots(3,2,figsize=(10,10))
        ax[0,0].imshow(np.argmax(test_acc[i][1][0],2))
        ax[0,1].imshow(np.argmax(test_acc[i][2][0],2))
        ax[1,0].imshow(np.argmax(test_acc[i][3][0],2))
