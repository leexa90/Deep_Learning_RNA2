...
'''
thres_distance_a,thres_distance_b,thres_distance_c,thres_distance_d,thres_distance_e, = 8,10,13,16,20
def make_array(str):
    temp = [0,]*len(str)
...
for i in data1_keys_train:
    if len(data1[i][0]) >= 35:
        if i in data2.keys():
            try:
                temp1 = data1[i]
                a,b,c = (data1[i][2] > thres_distance_a)*1,(data1[i][2] > thres_distance_b)*1,(data1[i][2] > thres_distance_c)*1
                d,e = (data1[i][2] > thres_distance_d)*1,(data1[i][2] > thres_distance_e)*1
                temp_resi_map = np.stack((a,b,c,d,e),axis=2)
                d = -1*(np.isnan(data1[i][2])-1) #non-nan values ==1 , nan =0
                d = np.stack((d,d,d,d,d),axis=2)
...
for i in data1_keys_test:
    if len(data1_t[i][0]) >= 35:
        if i in data2_t.keys():
            try:
                temp1 = data1_t[i]
                a,b,c = (data1_t[i][2] > thres_distance_a)*1,(data1_t[i][2] > thres_distance_b)*1,(data1_t[i][2] > thres_distance_c)*1
                d,e = (data1_t[i][2] > thres_distance_d)*1,(data1_t[i][2] > thres_distance_e)*1
                temp_resi_map = np.stack((a,b,c,d,e),axis=2)
...
resi_map0 = tf.placeholder(tf.float32,[None,None,None,5],name='distance_mat')
# some values are na, these will be excluded from loss
above_zero = tf.placeholder(tf.float32,[None,None,None,5],name='above_zero')
...
##    '4_wc1d': tf.Variable(tf.random_normal([1, 1, num3*4, 16])),
    ...
    '9_out2': tf.Variable(tf.random_normal([5,5,num5*4, 5]))   
}
...
biases = {
    ...
    '9_out2': tf.Variable(tf.random_normal([5]))
}
...
log_loss1 =tf.nn.sigmoid_cross_entropy_with_logits(logits = out[:,:,:,0:1], labels = resi_map[:,:,:,0:1] )
log_loss2 =tf.nn.sigmoid_cross_entropy_with_logits(logits = out[:,:,:,1:2], labels = resi_map[:,:,:,1:2] )
log_loss3 =tf.nn.sigmoid_cross_entropy_with_logits(logits = out[:,:,:,2:3], labels = resi_map[:,:,:,2:3] )
log_loss4 =tf.nn.sigmoid_cross_entropy_with_logits(logits = out[:,:,:,3:4], labels = resi_map[:,:,:,3:4] )
log_loss5 =tf.nn.sigmoid_cross_entropy_with_logits(logits = out[:,:,:,4:5], labels = resi_map[:,:,:,4:5] )
log_loss = tf.add(log_loss1,log_loss2)
for i in [log_loss3,log_loss4,log_loss5]:
    log_loss = tf.add(i,log_loss)
            ...
            for j in range(i+1,answer.shape[1]):
                if np.sum(answer[j,:]) != 0:
                    if answer[i,j] == mat_model[i,j]:
                        ...
                        score[answer[i,j]] += [0,]
    return np.mean([np.mean(score[0]),np.mean(score[1]),np.mean(score[0]),np.mean(score[1]),])
