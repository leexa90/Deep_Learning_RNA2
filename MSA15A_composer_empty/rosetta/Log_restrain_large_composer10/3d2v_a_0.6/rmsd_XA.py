...
import sys,os
sol= [x for x in os.listdir('.') if ('_' in x and 'pdb' in x and len(x.split('_')[0]) ==4)][0]
sys.path.append('../')
import alignment
import numpy as np
for num in [sol,]+sorted([x for x in os.listdir('.') if ('.pdb' in x and 'TPP'  in x)]):
    f1= open(sol,'r')
    f2= open('%s'%num,'r')
    first_resi = True
    cord = []
    seqP = ''
    for line in f1:
        if  'ATOM' in line and line[17:20].strip() in ['A','U','G','C'] and line[12:16].strip() == "C1'":
