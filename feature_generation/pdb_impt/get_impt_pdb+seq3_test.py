...
def print_file(cord,chain,i):#prints pdb file
...
for cluster in result2_resln:
    cluster = [ (2.0,'X-RAY DIFFRACTION', '5T5A|1|A'),]
    counter += 1
    completed = False
    for pdb in sorted(cluster):
        if completed is False and '+' not in pdb[2]:
            try:
                ...
                ii = ii.lower().strip()
                chain = ii[5:]
                x = 'GCAGGGCAAGGCCCAGUCCCGUGCAAGCCGGGACCGCCCCGGGGCGCGGCGCUCAUUCCUGC'
                pdb_seq0, full_seq0, cord = get_cif(ii,x)
                die
