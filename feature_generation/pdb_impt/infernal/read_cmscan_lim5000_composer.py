'''
../src/cmscan -Z 0.1   --textw 999 --toponly -E 10 --nohmmonly  --fmt 2 --clanin Rfam.clanin --tblout RNA_lim5000.tblout   Rfam.cm RNA_lim5000.fa >  RNA_lim5000.cmscan
'''
f1 = open('RNA_lim5000_composer.cmscan','r')
f2 = open('RNA_lim5000_composer.fa','r')
RNA_dictt = {}
for line in f2:
    if '>' in line:
        RNA_dictt[line[1:-1]] = ''
...
for i in RNA_dictt:
    ...
    if len(RNA_dictt[i])!=2 and i != name:
        print i
    else:
        dictt_result[i]  = []
        if 'assing' not in RNA_dictt[i][1][9]: # ensure an alignment is found
            ...
            if i == '3jd5_a': #from previous run
                remove_star = '**************************************************XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX**************************************************XXX************************************************************************************XXXX************************************************************************************************************************************************************XXXXXXXX******************************************************************************************************************************XXX**************************************************************XX***************************************************************************************************************************************************************************************************XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX*********************************************************'
            else:
...
import numpy as np
np.save('data_lim5000_MSA_composer.npy',dictt_result)
